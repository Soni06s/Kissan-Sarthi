...
const fallbackSummary = {
  user: { name: "Guest Farmer", location: "Bari Brahmana, J&K" },
  kpis: {
    temperature: { value: "28°C", trend: "+2°", sub: "High UV Index" },
    soilMoisture: { value: "64%", trend: "Critical", sub: "Field Area #03" },
    activeYield: { value: "14.2", trend: "79% Goal", sub: "Target: 18 Tons" },
    marketIndex: { value: "\u20b92,480", trend: "+5.2%", sub: "Samba Mandi" },
  },
  charts: {
  ...
  alerts: [
    { type: "warning", text: "Low moisture in field #3 - Consider irrigation", time: "12m ago" },
    { type: "info", text: "Wheat harvest optimal window: Nov 15-25", time: "1h ago" },
    { type: "success", text: "Soil health improved 12% this month", time: "3h ago" },
  ],
  aiInsights: [
    { title: "Yield Improvement", description: "Apply micro-nutrients during flowering." },
    { title: "Hydration Suggestion", description: "Drip irrigation saves 40% water vs flood." },
...
const DashboardPage = () => {
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      ...
      if (liveSummary) {
        setSummary({ ...fallbackSummary, ...liveSummary });
        setLastUpdated(new Date());
        setConnection("live");
      }
    } catch (error) {
      setConnection(error.response?.status === 401 ? "signin" : "offline");
    }
  }, []);

  useEffect(() => {
    fetchDashboard();
    const intervalId = window.setInterval(fetchDashboard, REFRESH_MS);
    return () => window.clearInterval(intervalId);
  }, [fetchDashboard]);

  const displayName = user?.name || fallbackSummary.user.name;
  const location = user ? `${user.city || 'Unknown'}, ${user.state || 'Location'}` : fallbackSummary.user.location;
  ...
  const statusBadge = useMemo(() => {
    if (connection === "live") return { text: "Live Data", color: COLORS.primary };
    if (connection === "signin") return { text: "Sign In For Live Data", color: COLORS.orange };
    if (connection === "offline") return { text: "Using Cached View", color: COLORS.accentDark };
    return { text: "Syncing Data", color: COLORS.blue };
  }, [connection]);

  const metricCards = [
    { icon: "sun", label: "Temperature", color: COLORS.orange, ...kpis.temperature },
    { icon: "drop", label: "Soil Moisture", color: COLORS.blue, ...kpis.soilMoisture },
    { icon: "crop", label: "Active Yield", color: COLORS.primary, ...kpis.activeYield },
    { icon: "market", label: "Market Index", color: COLORS.accentDark, ...kpis.marketIndex },
  ];
  ...
  return (
    <div className="page-transition" style={{ paddingBottom: "40px" }}>
      <div style={{
        ...
        <div>
          ...
          <p style={{ color: COLORS.textMuted, margin: "6px 0 0", fontSize: 16, fontWeight: 500 }}>
            Farm Command Center - <span style={{ color: COLORS.primary }}>{location}</span> - {new Date().toLocaleDateString("en-IN", { weekday: "long", month: "long", day: "numeric" })}
          </p>
        </div>
        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          <Badge text={statusBadge.text} color={statusBadge.color} />
          <Badge text="Kharif 2026" color={COLORS.blue} />
          {lastUpdated && (
      ...
      <div className="grid-container" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(200px, 1fr))", gap: 20, marginBottom: 25 }}>
        {metricCards.map((s) => (
          <Card key={s.label} style={{ padding: "24px", position: "relative", overflow: "hidden", display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }} title={s.label}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 15, gap: 12 }}>
              <div style={{ width: 52, height: 52, borderRadius: 16, background: s.color + "15", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <Icon name={s.icon} size={26} color={s.color} />
              </div>
              <span style={{ fontSize: 12, fontWeight: 800, color: s.color, background: s.color + "15", padding: "6px 12px", borderRadius: 12, height: "fit-content", display: 'flex', alignItems: 'center', gap: 4 }}>
                {s.trend === "79% Goal" ? <Icon name="trend" size={14} color={s.color} /> : null}
                {s.trend}
              </span>
            </div>
            ...
            {s.label === "Active Yield" ? (
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  <div style={{ fontSize: 34, fontWeight: 900, color: COLORS.text, fontFamily: "Georgia, serif" }}>{s.value}</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: COLORS.textMuted }}>Tons</div>
                </div>
                <div style={{ height: 6, background: "#e2e8f0", borderRadius: 3, marginTop: 12, marginBottom: 8, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: '79%', background: s.color, borderRadius: 3 }}></div>
                </div>
              </div>
            ) : (
              <div style={{ fontSize: 34, fontWeight: 900, color: COLORS.text, fontFamily: "Georgia, serif" }}>{s.value}</div>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: COLORS.textMuted, marginTop: 8 }}>{s.label}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20, marginBottom: 25 }} className="grid-split">
        <Card style={{ padding: "24px", display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
            <div>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: COLORS.text }}>Market Price Trend</h3>
            ...
            <div style={{ textAlign: "right", background: COLORS.primary + "10", padding: "8px 16px", borderRadius: 12 }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: COLORS.primary, textTransform: 'uppercase', letterSpacing: 0.5 }}>Highest Ask</div>
            </div>
          </div>
        ...
        <Card style={{ padding: "24px", display: 'flex', flexDirection: 'column' }}>
          <div style={{ marginBottom: 20 }}>
            <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: COLORS.text }}>Soil Health Monitor</h3>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: "auto" }}>
            <StatPill value={`${Math.round(latestMoisture)}%`} label="Moisture" icon="drop" color={COLORS.blue} />
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 24 }}>
              <div style={{ background: "rgba(255,255,255,0.15)", padding: "10px 14px", borderRadius: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 11, opacity: 0.8 }}>Humidity</div>
              ...
              <div style={{ background: "rgba(255,255,255,0.15)", padding: "10px 14px", borderRadius: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 11, opacity: 0.8 }}>Wind</div>
